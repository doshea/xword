...
## Recently Completed
- **Word/Clue Detail Pages polish (11 items)**: Fixed N+1 on `crosswords_by_title` (added `.includes(:user)` to Word, Clue, Phrase models). Added `- title` to both show pages. Replaced Ruby `sort_by(&:difficulty)` with SQL `.order(:difficulty)` in WordsController. Added word length subtitle to word page (`_word.html.haml`). Added word link + letter count to clue page. Added dot-based difficulty indicators (`●○` pattern) to word's clue list. Added `.xw-prose__subtitle` and `.xw-difficulty` CSS to `_components.scss`. Empty clue section guard added. Migrated 3 spec files from `should` to `expect()`/`is_expected.to`. Also updated words_controller_spec assertion from `be_an Array` to `respond_to(:to_a)` since clues is now an AR::Relation. 966 specs pass. No migration.
- **Profile page logic/UX review (7 items)**: Hidden "In Progress" stat from other users' profiles (`@own_profile` flag). Added turbo_stream responses to friend accept/reject (in-place update instead of redirect). Added "Edit Profile" ghost button on own profile (links to account settings). Simplified comment includes (removed dead 3rd-level preload). Simplified `Comment#base_crossword` from while-loop+Set to 2-level lookup. Avatar CSS 140→120px (matches upload size). Display `user.location` when present. 7 new specs (5 users, 2 friend_requests). No migration. 954 specs pass.
